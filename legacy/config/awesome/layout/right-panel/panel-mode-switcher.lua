...
local notif_button = clickable_container(
                         wibox.container.margin(notif_text, dpi(0), dpi(0),
                                                dpi(7), dpi(7)))
...
local today_button = clickable_container(
                         wibox.container.margin(today_text, dpi(0), dpi(0),
                                                dpi(7), dpi(7)))
...
local wrap_today = wibox.widget {
    ...
    border_color = beautiful.groups_title_bg,
    shape = function(cr, width, height)
        gears.shape.partially_rounded_rect(cr, width, height, true, false,
                                           false, true, beautiful.groups_radius)
    end,
    widget = wibox.container.background
}
