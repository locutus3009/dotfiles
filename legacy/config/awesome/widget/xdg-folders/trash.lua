...
local create_widget = function()
    ...
    local trash_menu = awful.menu({
        items = {
            {
                'Open trash', function()
                    awful.spawn.easy_async_with_shell('gio open trash:///',
                                                      function(stdout) end, 1)
                end, widget_icon_dir .. 'open-folder.svg'
            }, {
                'Delete forever', {
                    {
                        'Yes', function()
                            awful.spawn.easy_async_with_shell(
                                'gio trash --empty', function(stdout) end, 1)
                        end, widget_icon_dir .. 'yes.svg'
                    }, {'No', '', widget_icon_dir .. 'no.svg'}
    ...
    local trash_button = wibox.widget {
        {trash_widget, margins = dpi(10), widget = wibox.container.margin},
    ...
    local check_trash_list = function()
        awful.spawn.easy_async_with_shell('gio list trash:/// | wc -l',
                                          function(stdout)
            if tonumber(stdout) > 0 then
                trash_widget.trash_icon:set_image(widget_icon_dir ..
                                                      'user-trash-full.svg')

                awful.spawn.easy_async_with_shell('gio list trash:///',
                                                  function(stdout)
    ...
    awful.spawn.easy_async_with_shell(
        ...
        function()
            awful.spawn.with_line_callback('gio monitor trash:///', {
                stdout = function(_) check_trash_list() end
            })
        end)
