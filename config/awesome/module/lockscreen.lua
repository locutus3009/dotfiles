...
local locker_config = {
    ...
    bg_dir = config.module.lockscreen.bg_dir or
        (config_dir .. 'theme/wallpapers/'),
    -- Default background
    bg_image = config.module.lockscreen.bg_image or 'morning-wallpaper.jpg',
    -- /tmp directory
    tmp_wall_dir = config.module.lockscreen.tmp_wall_dir or '/tmp/awesomewm/' ..
        os.getenv('USER') .. '/'
}
...
local msg_table = {
    'This incident will be reported.', 'We are watching you.',
    'We know where you live.', 'RUN!', 'Yamete, Oniichan~ uwu',
    'This will self-destruct in 5 seconds!', 'Image successfully sent!',
    'You\'re doomed!', 'Authentication failed!', 'I am watching you.',
    'I know where you live.', 'RUN!', 'Your parents must be proud of you.'
}
...
local locker_arc = wibox.widget {
    ...
    shape = function(cr, width, height)
        gears.shape.arc(cr, width, height, dpi(5), 0, (math.pi / 2), false,
                        false)
    end,
    widget = wibox.container.background
}
...
local locker_widget = wibox.widget {
    {locker_arc, widget = rotate_container},
...
local locker = function(s)
    ...
    awful.placement.top(wanted_poster, {margins = {top = dpi(10)}})
            ...
            if key == 'Return' then
                ...
                if input_password ~= nil then
                    ...
                    if module_check('liblua_pam') then
                        ...
                        authenticated = input_password ==
                                            locker_config.fallback_password()
...
local filter_bg_image = function(wall_name, index, ap, width, height)
	...
	convert -quality 100 -brightness-contrast -20x0 ]] .. ' ' .. blur_filter_param ..
                      ' ' .. locker_config.bg_dir .. wall_name ..
                      [[ -gravity center -crop ]] .. ap ..
...
local apply_ls_bg_image = function(wall_name)
    ...
    for s in screen do
        ...
        local cmd = nil
        cmd = filter_bg_image(wall_name, index, aspect_ratio, screen_width,
                              screen_height)
        ...
        if s.index == 1 then
            ...
            awful.spawn.easy_async_with_shell(cmd, function()
                s.lockscreen.bgimage = locker_config.tmp_wall_dir .. index ..
                                           wall_name
            end)
        else
            -- Multihead screen/s
            awful.spawn.easy_async_with_shell(cmd, function()
                s.lockscreen_extended.bgimage =
                    locker_config.tmp_wall_dir .. index .. wall_name
            end)
        end
    end
