...
local notify_all_unread_email = function(email_data)
    ...
    local unread_counter = email_data:match('Unread Count: (.-)From:')
                               :sub(1, -2)
...
local set_email_data_tooltip = function(email_data)
    ...
    local counter = "<span font='Inter Regular 10'>Unread Count: </span>" ..
                        unread_email_count
...
local set_widget_markup = function(from, subject, date, tooltip)
    ...
    if tooltip then email_details_tooltip:set_markup(tooltip) end
end

local set_no_connection_msg = function()
    set_widget_markup('message@stderr.sh', 'Check network connection!',
                      os.date('%d-%m-%Y %H:%M:%S'), 'No internet connection!')
end

local set_invalid_credentials_msg = function()
    set_widget_markup('message@stderr.sh', 'Invalid Credentials!',
                      os.date('%d-%m-%Y %H:%M:%S'),
...
local set_latest_email_data = function(email_data)
    ...
    local recent_subject = email_data:match('Subject: (.-)Local Date:'):sub(1,
                                                                            -2)
    local recent_date = email_data:match('Local Date: (.-)\n')

    recent_from = recent_from:match('<(.*)>') or
                      recent_from:match('&lt;(.*)&gt;') or recent_from

    local count = tonumber(unread_count)
    if count > 0 and count <= 9 then
        email_icon_widget.icon:set_image(
            widget_icon_dir .. 'email-' .. tostring(count) .. '.svg')
    elseif count > 9 then
...
local set_empty_inbox_msg = function()
    set_widget_markup('empty@stdout.sh', 'Empty inbox',
                      os.date('%d-%m-%Y %H:%M:%S'), 'Empty inbox.')
end
...
local set_missing_secrets_msg = function()
    set_widget_markup('message@stderr.sh', 'Credentials are missing!',
                      os.date('%d-%m-%Y %H:%M:%S'), 'Missing credentials!')
end

local check_secrets = function()
    if secrets.email_address == '' or secrets.app_password == '' or
        secrets.imap_server == '' or secrets.port == '' then
...
local update_widget_timer = gears.timer {
    ...
    autostart = true,
    call_now = true,
    callback = function() check_secrets() end
}

email_report:connect_signal('mouse::enter', function() check_secrets() end)

awesome.connect_signal('system::network_connected', function()
    gears.timer.start_new(5, function() check_secrets() end)
end)
