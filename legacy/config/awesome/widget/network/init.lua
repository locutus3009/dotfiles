...
local return_button = function()
    ...
    local widget_button = wibox.widget {
        {widget, margins = dpi(7), widget = wibox.container.margin},
    ...
    local update_wireless = function()
        ...
        local notify_connected = function(essid)
            local message = 'You are now connected to <b>\"' .. essid ..
                                '\"</b>'
        ...
        local update_wireless_icon = function(strength)
            awful.spawn.easy_async_with_shell(check_internet_health,
                                              function(stdout)
                ...
                if not stdout:match('Connected but no internet') then
                    ...
                    widget_icon_name = widget_icon_name .. '-' ..
                                           tostring(strength) .. '-alert'
                    update_wireless_data(wifi_strength_rounded, false)
                end
                widget.icon:set_image(widget_icon_dir .. widget_icon_name ..
                                          '.svg')
            end)
        end
    ...
    local update_wired = function()
        ...
        awful.spawn.easy_async_with_shell(check_internet_health,
            ...
            if stdout:match('Connected but no internet') then
                ...
                update_tooltip('Ethernet Interface: <b>' ..
                                   interfaces.lan_interface .. '</b>')
                if startup or reconnect_startup then
                    awesome.emit_signal('system::network_connected')
                    notify_connected()
