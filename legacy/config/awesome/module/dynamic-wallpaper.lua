...
local wall_config = {
    ...
    wall_dir = filesystem.get_configuration_dir() ..
        (config.module.dynamic_wallpaper.wall_dir or 'theme/wallpapers/'),

    -- If there's a picture format that awesome accepts and i missed
    -- (which i probably did) feel free to add it right here
    valid_picture_formats = config.module.dynamic_wallpaper
        .valid_picture_formats or {"jpg", "png", "jpeg"},
...
-- Get current time
local current_time = function() return os.date('%H:%M:%S') end
...
local function parse_to_time(seconds)
    ...
    local function format(str)
        while #str < 2 do str = '0' .. str end

        return str
    end

    local function convert(num) return format(tostring(num)) end
...
local function get_dir_contents(dir)
    ...
    local files = {}
    for line in lines do table.insert(files, line) end
    return files
end
...
local function find_files_containing_keywords(files, keywords)
    ...
    for _, word in ipairs(keywords) do -- Preserves keyword order inherently, conveniently
        for _, file in ipairs(files) do
            ...
            if file == word or string.find(file, "^" .. word .. "[^%a]") or
                string.find(file, "[^%a]" .. word .. "[^%a]") then
...
local function auto_schedule(wall_list)
    ...
    for index, file in ipairs(wall_list) do
        local auto_time = parse_to_time(parse_to_seconds("24:00:00") *
                                            (index - 1) / #wall_list)
...
if #wall_config.wallpaper_schedule == 0 then
    ...
    for k, v in pairs(wall_config.wallpaper_schedule) do count = count + 1 end

    if count == 0 then -- Schedule is actually empty
        -- Get all pictures
        local pictures = filter_files_by_format(get_dir_contents(
    ...
    for _, word in ipairs(keywords) do
        ...
        if file ~= nil then table.insert(ordered_pictures, file) end
    end
...
local manage_timer = function()
    ...
    for time, wallpaper in pairs(wall_config.wallpaper_schedule) do
        ...
        if previous_time == '' or parsed_time > parse_to_seconds(previous_time) then
            if parsed_time <= time_now then previous_time = time end
        end
    ...
    if previous_time == '' then previous_time = last_time end

    -- Find next_time
    for time, wallpaper in pairs(wall_config.wallpaper_schedule) do
        local parsed_time = parse_to_seconds(time)
        if next_time == '' or parsed_time < parse_to_seconds(next_time) then
            if parsed_time > time_now then next_time = time end
        end
    ...
    if next_time == '' then next_time = first_time end
